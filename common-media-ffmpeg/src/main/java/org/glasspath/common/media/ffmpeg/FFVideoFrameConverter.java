...
public class FFVideoFrameConverter extends FrameConverter<BufferedImage> {

	public static boolean TODO_DEBUG = false;
	
	public static final int CONVERTER_DEFAULT = 0;
	public static final int CONVERTER_OPTIMIZED = 1;
	public static final int CONVERTER_TEST_1 = -1;
	public static final int CONVERTER_TEST_2 = -2;
	public static final int CONVERTER_TEST_3 = -3;

	static {
		//avutil.av_log_set_level(avutil.AV_LOG_QUIET); //Older JavaCV versions
	...
	private int converterType = CONVERTER_OPTIMIZED;
	private byte[] framePixels = null;
	private final ColorSpace colorSpace = ColorSpace.getInstance(ColorSpace.CS_sRGB);
	private final ColorModel colorModel = new ComponentColorModel(colorSpace, false, false, Transparency.OPAQUE, DataBuffer.TYPE_BYTE);
	...
	public BufferedImage createBufferedImage(Frame frame) {
		...
		if (converterType == CONVERTER_DEFAULT) {
		...
		} else if (converterType == CONVERTER_OPTIMIZED) {

			try {

				final ByteBuffer buffer = (ByteBuffer)frame.image[0].position(0);

				final BufferedImage image = new BufferedImage(frame.imageWidth, frame.imageHeight, FFVideoFrameConverter.getBufferedImageType(frame));
				final byte[] bufferPixels = ((DataBufferByte)image.getRaster().getDataBuffer()).getData();

				if (bufferPixels.length == buffer.limit()) {
					buffer.get(bufferPixels);
					return image;
				} else {
					System.err.println("VideoFrameConverter CONVERTER_OPTIMIZED, buffer lengths don't match..");
					converterType = CONVERTER_DEFAULT;
		...
		} else if (converterType == CONVERTER_TEST_3) {

			final ByteBuffer buffer = (ByteBuffer)frame.image[0].position(0);

			if (framePixels == null || framePixels.length < buffer.limit()) {
		...
		} else if (converterType == CONVERTER_TEST_2) {

			final int sizeBytes = frame.imageWidth * frame.imageHeight * 3;
			...
			final BufferedImage image = gc.createCompatibleImage(frame.imageWidth, frame.imageHeight, Transparency.TRANSLUCENT);
			final WritableRaster raster = image.getRaster();
			final DataBufferInt dataBuffer = (DataBufferInt)raster.getDataBuffer();

			//TODO?
			//RGBImageLoader.addAlphaChannel(framePixels, sizeBytes, dataBuffer.getData());

			return image;

		} else if (converterType == CONVERTER_TEST_1) {
			...
			if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_UBYTE || frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_BYTE) {
			...
			} else if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_USHORT) {
			...
			} else if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_SHORT) {
			...
			} else if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_INT) {
			...
			} else if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_FLOAT) {
			...
			} else if (frame.imageDepth == org.bytedeco.javacv.Frame.DEPTH_DOUBLE) {
			...
			final byte[] bufferPixels = ((DataBufferByte)wr.getDataBuffer()).getData();
			final BufferedImage image = new BufferedImage(cm, wr, false, null);

			 */

			/*
			final BufferedImage image = new BufferedImage(frame.imageWidth, frame.imageHeight, Java2DFrameConverter.getBufferedImageType(frame));
			Java2DFrameConverter.copy(frame, image, 1.0, false, null);
			 */

			final WritableRaster writableRaster = Raster.createWritableRaster(new ComponentSampleModel(DataBuffer.TYPE_BYTE, frame.imageWidth, frame.imageHeight, frame.imageChannels, frame.imageStride, new int[] {2, 1, 0}), null);
			final byte[] bufferPixels = ((DataBufferByte)writableRaster.getDataBuffer()).getData();

			System.arraycopy(framePixels, 0, bufferPixels, 0, framePixels.length);

			final BufferedImage image = new BufferedImage(colorModel, writableRaster, false, null);
			//image.setAccelerationPriority(1.0F);

			//final GraphicsDevice graphicsDevice = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
			//final BufferedImage image = graphicsDevice.getDefaultConfiguration().createCompatibleImage(frame.imageWidth, frame.imageHeight, Transparency.OPAQUE);
